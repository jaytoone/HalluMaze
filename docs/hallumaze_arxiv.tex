...
To establish external validity, we apply the identical AP prompt to public HumanEval and MBPP problems injected with the same trap categories: on HumanEval, pass@1 rises from 0.300 to 0.875 ($\Delta{=}{+}0.575$, $d{=}{+}1.16$, large effect); on MBPP, AP trap detection transfers (85.7\%) but base coding difficulty dominates pass@1, revealing difficulty level as a moderator of AP effectiveness.
...
\begin{itemize}[nosep]
  ...
  \item \textbf{External validity (HumanEval-Trap + MBPP-Trap)}: The identical AP prompt on HumanEval problems yields pass@1$=$0.875 vs.\ 0.300 ($\Delta{=}{+}0.575$, $d{=}{+}1.16$). On MBPP, AP trap detection transfers (85.7\%) but base difficulty dominates pass@1 --- revealing difficulty level as a moderator and confirming HumanEval as the appropriate difficulty-matched validation benchmark.
\end{itemize}
