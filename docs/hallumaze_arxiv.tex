...
For GLM-4.5-Air, AP achieves CodeMEI$=$0.812 vs.\ MARL-SL$=$0.737 ($+$0.075), with SR$=$100\% and HRR$=$82.4\% ($n{=}17$ valid).
...
The AP improvement effect is \textit{larger} for the weaker model ($+$0.156 vs.\ $+$0.075), suggesting that explicit adversarial awareness compensates for capacity limitations more effectively than structured role-playing.

\begin{table}[h]
\centering\small
\caption{HalluCode middleware ablation: Baseline, MARL-SL (5-layer), and AI Booster -- Adversarial Priming (AP). $n{=}19$ each; AP rows use valid-$n$ ($n{=}17$, excluding rate-limit errors). $\Delta$ vs.\ same-model Baseline.}
...
\multirow{3}{*}{GLM-4.5-Air}
 ...
 & \textbf{AP (AI Booster)} & \textbf{0.812} & 100.0\% & 82.4\% & $\mathbf{+0.233}$ & $+$21.1pp \\
\bottomrule
\end{tabular}
\end{table}
