...
A pilot study ($n{=}5$ per model, 2 free-tier models) demonstrates the SR$\perp$HRR dissociation in the coding domain: LFM-1.2B-Thinking achieves HRR$=$60\% yet SR$=$0\%, confirming that coding ability and metacognitive awareness are separable dimensions.
An unexpected finding (H5) shows that small thinking models (1.2B) outperform large generalist models ($\sim$7B) on wrong-signature trap detection (66.7\% vs.\ $\approx$0\%), suggesting that model size does not monotonically determine metacognitive quality---a pattern consistent with the SR$\neq$MEI inversion observed in HalluMaze.
Full HalluCode results (n$=$60, 5 models) are planned as a companion paper.
...
\begin{itemize}[nosep]
  ...
  \item \textbf{HalluCode pilot scale}: The HalluCode results reported in this paper are based on $n{=}5$ problems per model (2 free-tier models, 2 of 3 trap types). While sufficient to demonstrate the SR$\perp$HRR dissociation qualitatively and validate the H5 small-model hypothesis directionally, full statistical conclusions require $n{\geq}60$ per model with balanced trap-type coverage (nonexistent\_api, wrong\_signature, deprecated\_method). These are reported as preliminary findings pending a companion paper.
\end{itemize}
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{HalluCode (MRC Cubic for coding)}: Extension of the HalluMaze paradigm to coding environments via false API hint injection. Pilot (n$=$5, 2 models) confirms SR$\perp$HRR dissociation in coding domain. Full experiment (n$=$60, 5 models, 3 trap types) planned; executable at \texttt{scripts/run\_hallucode\_mvp.py}.
\end{enumerate}
