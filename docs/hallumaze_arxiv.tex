...
To establish external validity, we apply the identical AP prompt to 20 public HumanEval problems injected with the same 3 trap categories: pass@1 rises from 0.300 (baseline) to 0.875 (AP), $\Delta{=}{+}0.575$, $d{=}{+}1.16$ (large effect), confirming that awareness injection is domain-agnostic.
...
\begin{itemize}[nosep]
  ...
  \item \textbf{External validity (HumanEval-Trap)}: The identical AP prompt applied to 20 public HumanEval problems with injected false API hints yields pass@1$=$0.875 vs.\ 0.300 baseline ($\Delta{=}{+}0.575$, $d{=}{+}1.16$, large effect). This confirms AP is not a HalluCode-specific artifact---it is a domain-agnostic trap-awareness mechanism transferable to standard coding benchmarks.
\end{itemize}
