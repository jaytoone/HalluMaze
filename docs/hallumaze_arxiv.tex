...
\begin{enumerate}[nosep]
  ...
  \item \textbf{AI Booster v3 domain specificity} \textbf{[Validated, confirms trap-specificity]}: To test whether AP/AI Booster improves \emph{raw} coding ability (hypothesis H\textsubscript{AB}), we applied the same 3-step metacognitive middleware to \emph{trap-free} standard benchmarks. Qwen2.5-Coder-32B-Instruct (HF \#1 coding model, 926k downloads, 92.7\% published HumanEval) on \textbf{EvalPlus} (HumanEval$+$, NeurIPS 2023~\citep{evalplus2023}; 164 problems $\times$ 80$\times$ augmented test cases, $n{=}60$, seed$=42$): AI Booster pass@1$=$0.817, Baseline pass@1$=$0.817 ($\Delta{=}0.000$, McNemar $n_{01}{=}n_{10}{=}2$, $p{=}0.69$). \textbf{H\textsubscript{AB} is rejected}: AI Booster does not improve standard coding benchmarks. This \emph{confirms} the mechanism is trap-specific --- the VERIFY step targets hallucinated API calls, not general code quality. The large trap-setting effects ($\Delta{=}{+}0.575$ HumanEval-Trap; $\Delta{=}{+}0.235$ MBPP-Easy-Trap) are genuine domain-specific improvements, not contamination from general skill enhancement. Script: \texttt{scripts/run\_aibooster\_evalplus.py}.
...
A critical domain-specificity test further strengthens these claims: applying the same AI Booster middleware to \emph{trap-free} EvalPlus (HumanEval$+$, $n{=}60$, Qwen2.5-Coder-32B) yields $\Delta{=}0.000$ ($p{=}0.69$), confirming H\textsubscript{AB} is rejected. The middleware improves precisely what it targets---hallucinated API detection---without inflating raw coding scores, ruling out general prompt-engineering confounds. An Execution Feedback Booster (EFB; generate$\to$execute$\to$retry, AlphaCodium-inspired, $n{=}100$) yields $\Delta{=}{+}0.030$ ($p{=}0.19$), directionally consistent but ceiling-limited (89\% baseline pass@1 leaves only 11 failures), further confirming that performance interventions require genuine headroom to demonstrate statistical significance.
