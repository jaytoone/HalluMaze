...
We further introduce \textbf{HalluCode}, extending the paradigm to coding via false API hint injection (19 problems, 3 trap types).
...
External validity is confirmed on public benchmarks: AP raises HumanEval pass@1 from 0.300 to 0.875 ($\Delta{=}{+}0.575$, $d{=}{+}1.16$); on MBPP, AP detection transfers (85.7\%) while task difficulty moderates pass@1 benefit.
...
\begin{itemize}[nosep]
  ...
  \item \textbf{External validity (multi-benchmark + Spearman)}: AP Booster on HumanEval-Trap: pass@1$=$0.875 vs.\ 0.300 ($\Delta{=}{+}0.575$, $d{=}{+}1.16$). MBPP-Trap confirms AP detection transfers (85.7\%) with task difficulty as moderator. Spearman $\rho({\text{HumanEval}},\,{\text{MEI}}){=}{+}0.30$ ($p{=}0.32$, $n{=}13$) --- metacognitive recovery is \textbf{independent of coding ability}, with GPT-4o (HumanEval 90.2\%) ranking last on MEI.
\end{itemize}
