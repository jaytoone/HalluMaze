...
To establish external validity, we apply the identical AP prompt to public HumanEval and MBPP problems injected with the same trap categories: on HumanEval, pass@1 rises from 0.300 to 0.875 ($\Delta{=}{+}0.575$, $d{=}{+}1.16$, large effect); on MBPP, AP trap detection transfers (85.7\%) but base coding difficulty dominates pass@1, revealing difficulty level as a moderator.
Critically, Spearman correlation between our 13-model MEI leaderboard and published HumanEval scores yields $\rho{=}{+}0.30$ ($p{=}0.32$, not significant), indicating that metacognitive recovery is \emph{largely independent of general coding ability}---GPT-4o (HumanEval 90.2\%) ranks last on MEI (0.315), confirming HalluMaze captures a distinct capability dimension.
