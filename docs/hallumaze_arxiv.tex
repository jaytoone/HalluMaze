...
\begin{enumerate}[nosep]
  ...
  \item \textbf{AP Booster v2 + MBPP-Easy}: MBPP-Trap (AP v1) confirms awareness transfer (detection$=$85.7\%) but pass@1$=$0.000 is difficulty-moderated---GLM-4.5-Air free-tier cannot solve full MBPP even absent traps. AP Booster v2 addresses this with a 3-step prompt: (1)~Detect fake API (EXISTS / DOES\_NOT\_EXIST / WRONG\_USAGE), (2)~Name the correct stdlib alternative explicitly, (3)~Write code using verified API only. Paired with an easy-filtered MBPP subset (reference solution ${\leq}$7 non-empty lines, ${\leq}$2 test assertions) to decouple difficulty from trap-awareness, isolating the AP effect. Experimental script: \texttt{scripts/run\_mbpp\_easy\_trap.py}.
\end{enumerate}
...
External validity is confirmed across two public benchmarks: on HumanEval-Trap, the identical AP prompt raises pass@1 from 0.300 to 0.875 ($\Delta{=}{+}0.575$, $d{=}{+}1.16$); on MBPP-Trap, AP detection transfers (85.7\%) while task difficulty moderates pass@1---a boundary condition that motivates AP Booster v2 with explicit stdlib-alternative guidance on a difficulty-filtered MBPP-Easy subset (\texttt{run\_mbpp\_easy\_trap.py}).
