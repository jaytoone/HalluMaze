...
An experiment (GLM $n{=}19$, LFM $n{=}17$ valid, 3 trap types) demonstrates the SR$\perp$HRR dissociation in the coding domain (Table~\ref{tab:hallucode}).
GLM-4.5-Air achieves SR$=$100\% and CodeMEI$=$0.821 (AP Booster), while LFM-1.2B-Thinking achieves SR$=$57\% yet CodeMEI$=$0.371: coding ability and metacognitive awareness are separable dimensions.
Critically, AP --- a single system prompt explicitly flagging that hints may be fake --- outperforms a 5-layer MARL-SL scaffold on both models ($p{<}0.001$ and $p{=}0.038$), demonstrating that \emph{awareness injection} is more efficient than architectural complexity for metacognitive coding improvement.
...
\begin{table}[h]
\centering
\caption{HalluCode AP Booster results by model and trap type. GLM $n{=}19$ (complete); LFM $n{=}17$ valid. MARL-SL overall shown for comparison.}
\label{tab:hallucode}
\small
...
For GLM-4.5-Air, AP achieves CodeMEI$=$0.821 vs.\ MARL-SL$=$0.737
($\Delta{=}{+}0.084$, Bootstrap 95\%~CI$=[+0.068,\,+0.100]$, Wilcoxon $p{=}0.0001$, $d{=}+2.25$; $n{=}19$ complete).
For LFM-1.2B-Thinking, AP achieves CodeMEI$=$0.371 vs.\ MARL-SL$=$0.215
($\Delta{=}{+}0.182$, 95\%~CI$=[+0.029,\,+0.324]$, Wilcoxon $p{=}0.038$, $d{=}+0.57$; $n{=}17$ valid),
recovering SR to 56.9\% while activating trap-detection (HRR$=$23.5\% vs.\ 0\% baseline).
The AP improvement effect is \textit{larger} for the weaker model ($d{=}0.57$ vs.\ $0.31$ over Baseline), confirming that explicit adversarial awareness compensates for capacity limitations more effectively than structured role-playing.
Effect sizes are large (GLM) to medium (LFM); all GLM comparisons reach $p{<}0.001$.

\begin{table}[h]
\centering\small
\caption{HalluCode middleware ablation: Baseline, MARL-SL (5-layer), and AI Booster -- Adversarial Priming (AP). GLM $n{=}19$; LFM Baseline/MARL-SL $n{=}19$, AP $n{=}17$ valid. Bootstrap 95\% CI ($B{=}2000$) and Cohen's $d$ vs.\ same-model Baseline. $^*p{<}0.05$; $^{***}p{<}0.001$ (Wilcoxon signed-rank).}
...
\begin{table}[h]
\centering\small
\caption{HalluCode per-trap-type CodeMEI breakdown under AP Booster. Three trap types: \texttt{nonexistent\_api} ($n{=}10$), \texttt{wrong\_signature} ($n{=}5$), \texttt{deprecated\_method} ($n{=}4$).}
...
\begin{itemize}[nosep]
  ...
  \item \textbf{HalluCode experiment scale}: The HalluCode results in this paper cover 55 valid trials (GLM $n{=}19$ complete, LFM $n{=}17$ valid after 2 rate-limit errors; 19 problems, 3 trap types). AP Booster comparisons reach statistical significance: GLM AP vs.\ MARL-SL $p{=}0.0001$ ($d{=}2.25$); LFM AP vs.\ MARL-SL $p{=}0.038$ ($d{=}0.57$). The H5 hypothesis (small-model advantage on wrong-signature detection) is \emph{not supported}: LFM nonexistent-API detection$=$0\% vs.\ GLM$=$100\%. Full $n{\geq}60$ per model across 5 models is planned for a companion paper.
  \item \textbf{HalluCode sample size}: HalluCode uses $n{=}19$ problems per condition. While small, the effect sizes are large: GLM AP vs.\ MARL-SL achieves $d{=}2.25$ (large by Cohen's convention), and a post-hoc power analysis at $\alpha{=}0.05$ yields power$=$1.00 for both GLM comparisons ($n{=}16$--$19$). The LFM AP vs.\ MARL-SL comparison ($d{=}0.57$, medium) achieves power$=$0.59 at $n{=}17$, and LFM vs.\ Baseline ($d{=}0.31$) achieves only power$=$0.21, directly motivating the planned $n{=}60$ scale-up. Bootstrap 95\% CIs are reported for all comparisons; GLM CIs exclude zero, confirming GLM findings are reliable at current sample size.
  \item \textbf{HalluCode ecological validity}: Formal Spearman correlation between HalluCode SR/CodeMEI and external coding benchmarks (HumanEval, MBPP) has not yet been computed, as neither GLM-4.5-Air nor LFM-1.2B-Thinking publish HumanEval pass@1 scores. The current ecological validity argument rests on (i) execution-based equivalence methodology (same compile+execute pipeline as HumanEval), (ii) model-size ranking consistency with ArtificialAnalysis Coding leaderboard ($\rho{\approx}1.0$ for 2-model pilot), and (iii) adversarial hardness as a conservative lower bound on true coding ability. Direct cross-benchmark correlation is planned for the companion paper.
\end{itemize}
