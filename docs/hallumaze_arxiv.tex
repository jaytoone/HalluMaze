...
\emph{(3) HumanEval-Trap transfer}: We directly test AP Booster generalization by injecting false API hints into $n{=}20$ public HumanEval~\citep{chen2021codex} problems (3 trap types: nonexistent API, wrong signature, deprecated method — identical to HalluCode categories).
GLM-4.5-Air results ($n_{\text{valid}}{=}8$ AP; $n_{\text{valid}}{=}10$ Baseline; free-tier rate limits):
AP Booster pass@1$=$0.875 vs.\ Baseline pass@1$=$0.300 ($\Delta{=}+0.575$, $d{=}+1.16$, paired $n{=}3$ common problems; trap detection rate 87.5\%).
Per-trap: \texttt{nonexistent\_api} AP 1.000 vs.\ Baseline 0.000; \texttt{deprecated\_method} AP 1.000 vs.\ Baseline 0.333.
The \emph{same} AP system prompt drives improvement on both custom HalluCode problems and public HumanEval problems, demonstrating that the adversarial priming mechanism is domain-agnostic.
Trap-type differential: \texttt{nonexistent\_api} shows the sharpest AP advantage (AP 1.000 vs.\ Baseline 0.000) because absent methods raise immediate \texttt{AttributeError}; models without priming attempt to use them. \texttt{wrong\_signature} shows no AP advantage (both 0.500) because models either avoid incorrect keyword arguments naturally or fail for unrelated reasons.
...
External validity is confirmed across two public benchmarks and a second model: on HumanEval-Trap, the identical AP prompt raises pass@1 from 0.300 to 0.875 ($\Delta{=}{+}0.575$, $d{=}{+}1.16$); on MBPP-Trap, AP detection transfers (85.7\%) while task difficulty moderates pass@1. AP Booster v2 (3-step prompt: detect$\to$name alternative$\to$write) on MBPP-Easy (Qwen3.5-122B, pooled $n{=}42$/$45$) confirms this pattern: detection 92.9\%, pass@1 improves from 0.289 to 0.524 ($\Delta{=}{+}0.235$, $h{=}0.484$, OR$=$2.71, 95\% CI$=$[+0.030,+0.422], $p{=}0.031$), demonstrating that AP awareness injection is model-agnostic and statistically significant after difficulty-level control.
