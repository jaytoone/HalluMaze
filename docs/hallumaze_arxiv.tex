...
\begin{enumerate}[nosep]
  ...
  \item \textbf{AP Booster v2 + MBPP-Easy} \textbf{[Pilot completed]}: MBPP-Trap (AP v1) confirms awareness transfer (detection$=$85.7\%) but pass@1$=$0.000 is difficulty-moderated. AP Booster v2 uses a 3-step prompt: (1)~Detect fake API, (2)~Name correct stdlib alternative, (3)~Write code using verified API. Applied to MBPP-Easy (reference solution ${\leq}$7 lines) with Qwen3.5-122B ($n{=}13$ AP, $n{=}15$ baseline): detection$=$92.3\%, pass@1 improves from 0.333 to 0.538 ($\Delta{=}{+}0.205$, $h{=}0.417$, $p{=}0.24$ n.s., underpowered pilot). AP detection persists across difficulty levels; pass@1 benefit is difficulty-moderated. Full validation ($n{\geq}30$) pending. Script: \texttt{scripts/run\_mbpp\_easy\_trap.py}.
\end{enumerate}
...
External validity is confirmed across two public benchmarks and a second model: on HumanEval-Trap, the identical AP prompt raises pass@1 from 0.300 to 0.875 ($\Delta{=}{+}0.575$, $d{=}{+}1.16$); on MBPP-Trap, AP detection transfers (85.7\%) while task difficulty moderates pass@1. AP Booster v2 (3-step prompt: detect$\to$name alternative$\to$write) on MBPP-Easy (Qwen3.5-122B, $n{=}13$) confirms this pattern: detection 92.3\%, pass@1 improves from 0.333 to 0.538 ($\Delta{=}{+}0.205$, $h{=}0.417$), demonstrating that AP awareness injection is model-agnostic and difficulty-moderated rather than model-specific.
