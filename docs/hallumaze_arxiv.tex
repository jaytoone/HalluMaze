...
\noindent where:
\begin{itemize}[nosep]
  \item \textbf{HRR} (Hallucination Recovery Rate): $P(\text{correct backtrack} \mid \text{hallucination})$.
...
\subsection{Why Random Walk Wins}

The random walk achieves MEI$=$0.900 because it never ``believes'' its moves will succeed---it simply tries until one works, achieving HRR$=$1.0 by construction.
LLMs, by contrast, form confident beliefs that become anchored even under contradictory feedback.
This reveals a \textbf{metacognitive anchoring bias}: LLMs trained on text prediction learn to maintain confident world models rather than updating them efficiently under environmental contradiction.
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Temperature stochasticity}: Test-retest reliability (ICC) not yet measured.
  \item \textbf{No 9$\times$9 condition}: Size scaling study not yet complete.
\end{itemize}

\subsection{Multi-Agent Reasoning Layers}
...
\begin{enumerate}[nosep]
  ...
  \item ICC reliability (3 models $\times$ 10 seeds $\times$ 2 runs, target ${>}0.8$).
  \item Ecological validity (Spearman correlation with TruthfulQA/HaluEval public scores).
  \item 9$\times$9 maze condition (size scaling study).
...
A hallucination event is logged when:
(a) model asserts confidence $\geq 60$\% for a direction AND environment rejects, or
(b) model refuses with confidence $\geq 60$\% AND the mirage trigger fires.
