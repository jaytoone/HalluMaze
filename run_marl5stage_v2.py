#!/usr/bin/env python3
"""
MARL 5-Stage v2 — HalluMaze Benchmark
======================================
Fixes applied over v1:
  Fix 1: Path Validator Gate — deterministic validation of S2 path against maze walls
  Fix 2: Conditional Pipeline — retry S2 up to 3x if path invalid, pick best
  Fix 3: Maze Context Re-injection — S3/S4/S5 always see original maze text

Usage:
    python run_marl5stage_v2.py
"""
...
def validate_path(extracted_path: list, maze: MazeEngine) -> list[str]:
    ...
    Returns list of error strings. Empty list = valid path.
    Uses actual wall data (cells), NOT mirage-augmented text.
    Cell.N/S/E/W = True means wall exists (blocked).
    """
    ...
    for i in range(len(extracted_path) - 1):
        ...
        # Check if wall exists (True = wall = blocked)
...
def extract_path_from_text(text: str) -> list[list[int]]:
    ...
    # Fallback: extract from STEP records
...
def run_marl5stage_v2(maze: MazeEngine, size: int) -> dict:
    """Run 5-stage pipeline with Fix 1+2+3. Returns dict with all stage outputs + parsed metrics."""
...
Be specific with cell coordinates."""
    ...
    # ── S2: Solver (with retry gate — Fix 2) ──
    s2_system = (
...
FINAL_PATH: (0,0)->...->({N-1},{N-1})"""
        ...
        if len(errors) == 0:
            break  # Fix 2: valid path, proceed
    ...
    # ── S3: Auditor — Fix 3: maze re-injection ──
    s3_system = (
...
- ERROR at STEP N: (r,c) -> [direction] — [what's wrong] — FIX: [correction]"""
    ...
    # ── S4: Verifier — Fix 3: maze re-injection ──
    s4_system = (
...
FINAL_PATH: (0,0)->...->({N-1},{N-1})"""
    ...
    # ── S5: Refiner — Fix 3: maze re-injection ──
    s5_system = (
...
def main():
    ...
    checkpoint_path = os.path.join(
        ...
        "experiment_results", "marl5stage_v2_minimax.json",
    )
    ...
    # ── Final Analysis ──
    print("\n" + "=" * 60)
    print("  MARL 5-Stage v2 Results Summary")
    print("=" * 60)
    ...
    print(f"\n  Trials: {n}")
    print(f"\n  {'Metric':<25} {'v2':>10} {'v1':>10} {'Baseline':>10}")
    print(f"  {'-'*55}")
    print(f"  {'MEI':.<25} {mean_mei:>10.4f} {v1_mei:>10.4f} {baseline_mei:>10.4f}")
    print(f"  {'HRR':.<25} {mean_hrr:>10.4f} {'0.556':>10} {'0.600':>10}")
    print(f"  {'Solve Rate':.<25} {mean_sr:>10.4f} {'0.333':>10} {'0.533':>10}")
    ...
    # Comparison table
    print(f"\n  {'='*60}")
    print(f"  {'Method':<20} | {'MEI':>6} | {'SR':>6} | {'HRR':>6} | {'n':>3}")
    print(f"  {'-'*60}")
    print(f"  {'Baseline':<20} | {baseline_mei:>6.3f} | {'53.3%':>6} | {'0.600':>6} | {'60':>3}")
    print(f"  {'MARL v1':<20} | {v1_mei:>6.3f} | {'33.3%':>6} | {'0.556':>6} | {'9':>3}")
    sr_pct = f"{mean_sr*100:.1f}%"
    print(f"  {'MARL v2 (fixed)':<20} | {mean_mei:>6.3f} | {sr_pct:>6} | {mean_hrr:>6.3f} | {n:>3}")
    print(f"  {'='*60}")
    ...
    if mean_mei > v1_mei:
        print(f"  >>> v2 IMPROVES over v1 by {mean_mei - v1_mei:+.4f} ({(mean_mei/v1_mei - 1)*100:+.1f}%)")
    else:
        print(f"  >>> v2 vs v1: {mean_mei - v1_mei:+.4f} ({(mean_mei/v1_mei - 1)*100:+.1f}%)")
    ...
    _save_checkpoint(checkpoint_path, results, summary={
        ...
        "delta_vs_v1": round(mean_mei - v1_mei, 4),
    })
